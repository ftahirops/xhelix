...
**Doc order:** most-recent verified 2025-2026 incidents first (**#1–#10**), then historical and architectural reference attacks (**#11–#32**). Scenarios #1–#10 are sourced from live web search of public vendor advisories, CISA KEV, and security-vendor reports — citations in the Sources section at the end.

Detection rules referenced are bundled in `ruleset/core/*.yaml` and `ruleset/correlator/*.yaml` and shipped in the deb.

---

## 1. CVE-2026-45321 — "Mini Shai-Hulud" self-spreading npm worm (May 11, 2026)
...
## 2. CVE-2026-41940 — cPanel/WHM Authentication Bypass (Feb-April 2026)
...
## 3. CVE-2026-31431 — Linux Kernel "Copy Fail" zero-day (April-May 2026)
...
## 4. Linux "Dirty Frag" zero-day (early 2026)
...
## 5. CVE-2025-30066 — tj-actions/changed-files GitHub Action compromise (March 2025)
...
## 6. CVE-2026-20131 — Cisco Secure Firewall Management Center (FMC) zero-day (Jan-March 2026)
...
## 7. Microsoft durabletask PyPI compromise (May 19, 2026)
...
## 8. TrapDoor cross-ecosystem worm (May 22, 2026)
...
## 9. CVE-2026-44477 — CloudNativePG PostgreSQL RCE (2026)
...
## 10. CVE-2026-27825 — Atlassian MCP / Confluence SSRF→RCE (May 2026)
...
## 11. XZ Utils backdoor (CVE-2024-3094, March 2024)
...
## 12. SolarWinds Orion / Sunburst (December 2020)
...
## 13. 3CX desktop app supply-chain (March 2023)
...
## 14. Log4Shell exploitation chain (CVE-2021-44228, Dec 2021)
...
## 15. MOVEit zero-day mass exploit (CL0P, May-June 2023)
...
## 16. Capital One AWS metadata SSRF (2019)
...
## 17. Cortex-c2 implant family (open-source modern C2)
...
## 18. TeamTNT / Megalodon cryptominer dropper
...
## 19. SSH brute-force + key implantation (Mirai-style)
...
## 20. Container escape via privileged mount (CVE-2019-5736 class)
...
## 21. Memory implant / fileless malware (BPF or memfd-execve)
...
## 22. PAM module replacement (sudo backdoor)
...
## 23. event-stream / flatmap-stream npm supply chain (November 2018)
...
## 24. ua-parser-js + coa + rc npm hijack (October-November 2021)
...
## 25. PyTorch torchtriton typosquat (December 2022)
...
## 26. Codecov bash uploader compromise (April 2021)
...
## 27. Apache Struts / Equifax breach (CVE-2017-5638, March 2017)
...
## 28. Atlassian Confluence OGNL injection (CVE-2022-26134, June 2022)
...
## 29. Spring4Shell (CVE-2022-22965, March 2022)
...
## 30. Apache ActiveMQ RCE (CVE-2023-46604, October 2023)
...
## 31. F5 BIG-IP iControl REST RCE (CVE-2022-1388, May 2022)
...
## 32. Polyfill.io CDN compromise (June 2024)
...
|---|---|---|---|---|
| 1 | **CVE-2026-45321 Mini Shai-Hulud npm worm** (May 2026) | postinstall secret-read + outbound | Class 2-3 | `dropped_binary_lifecycle` + `secrettaint` + `egressguard` ensemble |
| 2 | **CVE-2026-41940 cPanel/WHM auth bypass** (Feb-Apr 2026) | cpsrvd spawning shell + persistence drop | Class 1 (`brp.hard_deny`) | Auth bypass invisible; post-exploit chain caught |
| 3 | **CVE-2026-31431 Linux Copy Fail kernel** (Apr-May 2026) | container→root escape + cross-pod pivot | Class 2-3 | Kernel primitive silent; pivot is loud |
| 4 | Linux Dirty Frag (2026) | kernel LPE → cap.gained + uid0_transition | Class 3 | Catches the transition, not the primitive |
| 5 | **CVE-2025-30066 tj-actions/changed-files** (Mar 2025) | env-secret scrape + outbound | Class 2 | Built for this exact CI/CD pattern |
| 6 | **CVE-2026-20131 Cisco FMC zero-day** (Jan-Mar 2026) | mgmt JVM spawning shell + ransomware persistence | Class 1 + Class 2 | Linux-host observability appliance lacks |
| 7 | **Microsoft durabletask PyPI** (May 19, 2026) | 90-path secret burst + IMDS + outbound | 3 separate Class 1/2 fires | Multi-cloud credential-stealer threat model |
| 8 | **TrapDoor cross-ecosystem worm** (May 22, 2026) | quadruple persistence + lateral SSH | 5+ named detectors fire | Best demonstration of detector composability |
| 9 | **CVE-2026-44477 CloudNativePG** (2026) | postgres spawning shell + kube SA token theft | Class 1 (`brp.hard_deny`) | DB role contract is unambiguous |
| 10 | **CVE-2026-27825 MCPwnfluence** (May 2026) | SSRF → off-path write → exec | Class 1 + Class 3 | Egressguard kills SSRF; FIM catches the write |
| 11 | XZ Utils backdoor | unauthorized child of sshd | Class 1 (`brp.hard_deny`) | Signed contract wins where signatures fail |
| 12 | SolarWinds Sunburst | C2 + recon + exfil | Class 1-2 | 12-day dormancy gap closed by Phase H.2 |
| 13 | 3CX desktop trojan | undeclared outbound peer | Class 1-2 | Same defense as supply-chain class |
| 14 | Log4Shell | JVM spawning shell | Class 1 | Textbook xhelix win |
| 15 | MOVEit zero-day | web role spawning shell + bulk DB read + undeclared exfil | Class 1-2 | Post-exploit chain detected without signature |
| 16 | Capital One IMDS SSRF | metadata access by non-cloud-role | Class 1 (`metadata.access_by_unexpected`) | One of the rules xhelix was built for |
| 17 | Cortex-c2 implant | 6+ independent stages | Class 1-3 | Specific chain rule (`dropped_binary_lifecycle`) for this class |
| 18 | TeamTNT cryptominer | credential burst + miner exec + persist | Class 2-3 | `file_read_burst` is built for this |
| 19 | SSH brute + key implant | N failures / window + authorized_keys write | Class 2-3 | Phase J.1 + canonical FIM detector |
| 20 | Container escape | privileged mount / pivot_root | Class 1-2 | `contescape.detected` + `cap.gained` |
| 21 | Memory implant (memfd) | execveat from memfd + RWX mprotect | Class 3 (multi-detector) | Strongest case for xhelix vs fileless |
| 22 | PAM module replacement | unauthorized write to security lib | Class 3 + verifier escalate | Anchored + chain-attributed |
| 23 | event-stream npm (2018) | postinstall secret-read + outbound | Class 2 | Phase K.2 closes pkg-install FP gap |
| 24 | ua-parser-js + coa + rc npm (2021) | preinstall fetch + drop + exec | Class 3 (`dropped_binary_lifecycle`) | Built for this class |
| 25 | PyTorch torchtriton (2022) | mass secret read + DNS exfil | Class 2 (`file_read_burst` + `dnsexfil`) | `procscrape` catches the env-read |
| 26 | Codecov bash uploader (2021) | `procscrape` env-read + outbound | Class 2 | Phase N broker mediation extends this |
| 27 | Apache Struts CVE-2017-5638 | JVM spawning shell | Class 1 (`brp.hard_deny`) | Canonical web-RCE detection |
| 28 | Confluence OGNL CVE-2022-26134 | OGNL → shell + persistence drop | Class 1 + Class 2 (two independent fires) | Two-stage redundancy |
| 29 | Spring4Shell CVE-2022-22965 | webroot write + shell spawn | Class 1 + Class 3 (two-stage) | Webroot write tag is the early signal |
| 30 | Apache ActiveMQ CVE-2023-46604 | undeclared egress + JVM-spawn-shell + chain | 3 independent fires | Pre-persistence detection |
| 31 | F5 BIG-IP CVE-2022-1388 | mgmt-spawning-shell + signing-key read + config wipe | Class 1 + Class 2 | Linux-level observation appliance doesn't expose |
| 32 | polyfill.io CDN (June 2024) | partial — server-side egress + cert SAN drift (Phase K.3 planned) | partial | Honest gap on client-side; server-side covered |
...
## Sources for scenarios #1–#10 (verified recent 2025-2026 incidents)

Verified via live web search 2026-05-27. All CVE numbers, dates, and attack details for scenarios #1 through #10 are traceable to:
