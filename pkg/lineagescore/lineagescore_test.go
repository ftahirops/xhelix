...
func TestEngine_CrossThreshold_EmitsOnce(t *testing.T) {
	...
	// Weight 10 keeps cumulative score at 110 — still tier "high", no
	// escalation — so same-tier re-fire stays suppressed within cooldown.
	// (Weight 50 here would push to 150 = critical, a genuine high->critical
	// escalation that re-emits by design; see TestEngine_TierEscalation_*.)
	if again := e.Observe(Signal{PID: 7, RuleID: "c", Weight: 10, At: now}); again != nil {
...
func TestEngine_TierEscalation_ReEmitsWithinCooldown(t *testing.T) {
	now := time.Unix(1000, 0)
	e := New(Opts{Threshold: 80, Window: time.Hour, Cooldown: time.Hour, LineageOf: fixedLineage(1)})
	e.Observe(Signal{PID: 5, RuleID: "a", Weight: 50, At: now})
	v1 := e.Observe(Signal{PID: 5, RuleID: "b", Weight: 50, At: now}) // 100 -> high
	if v1 == nil || v1.Tier != "high" {
		t.Fatalf("first crossing should be high, got %+v", v1)
	}
	// Same cooldown window, but a heavy signal pushes to critical.
	v2 := e.Observe(Signal{PID: 5, RuleID: "c", Weight: 80, At: now.Add(time.Minute)}) // 180 -> critical
	if v2 == nil {
		t.Fatal("tier escalation high->critical must re-emit even within cooldown")
	}
	if v2.Tier != "critical" {
		t.Fatalf("escalation verdict must be critical, got %q (score %d)", v2.Tier, v2.Score)
	}
	// A further signal that does NOT raise the tier must stay suppressed.
	v3 := e.Observe(Signal{PID: 5, RuleID: "d", Weight: 10, At: now.Add(2 * time.Minute)}) // 190 still critical
	if v3 != nil {
		t.Fatal("same-tier re-fire within cooldown must stay suppressed")
	}
}
