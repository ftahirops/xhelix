...
// TestEgressALPNClassifiesGRPC is the EO.5b contract: a flow whose
// client-offered ALPN (captured by the dpi sniffer into connstate, or
// surfaced via the alpn tag) advertises grpc is classified as grpc.
func TestEgressALPNClassifiesGRPC(t *testing.T) {
	led, err := egressledger.New(egressledger.Options{Dir: t.TempDir()})
	if err != nil {
		t.Fatalf("new ledger: %v", err)
	}
	...
	grpc := model.NewEvent("ebpf.net", model.SeverityInfo)
	grpc.PID = 4242
	grpc.Image = "/usr/bin/app"
	grpc.Comm = "app"
	grpc.Tags["kind"] = "net_connect"
	grpc.Tags["protocol"] = "tcp"
	grpc.Tags["dst_ip"] = "9.9.9.9"
	grpc.Tags["dst_port"] = "443"
	grpc.Tags["sni"] = "svc.example.com"
	grpc.Tags["alpn"] = "grpc"
	p.Handle(context.Background(), grpc)
	...
	if got != "grpc" {
		t.Fatalf("ALPN grpc L7Protocol = %q, want grpc", got)
	}
}
